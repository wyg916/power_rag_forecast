...
def _matches_filters(user: dict[str, Any], keyword: str = "", role: str | None = None, is_active: bool | None = None) -> bool:
    if keyword:
        ...
        conn.execute(
            text(
                ...
                INSERT INTO users (
                    ...
                    role, role_id, status, is_active, is_superuser, updated_at
                )
                VALUES (
                    :user_id, :username, :email, :display_name, :password_hash,
                    :role, :role_id, :status, :is_active, :is_superuser, CURRENT_TIMESTAMP
                )
...
def set_user_active(user_id: str, is_active: bool) -> dict[str, Any] | None:
    return update_user(user_id, is_active=bool(is_active))
...
def count_admin_users(*, active_only: bool = True) -> int:
    ...
    if engine is None:
        return sum(
            1
            for user in _MEMORY_USERS.values()
            if normalize_role(str(user.get("role") or "")) == "admin" and (not active_only or bool(user.get("is_active", True)))
        )
    where_active = "AND is_active = TRUE" if active_only else ""
    try:
        with engine.connect() as conn:
            value = conn.execute(
                text(f"SELECT COUNT(*) FROM users WHERE COALESCE(role, role_id) = 'admin' {where_active}")
            ).scalar_one()
...
def ensure_not_last_admin_demoted_or_disabled(user_id: str, *, new_role: str | None = None, new_is_active: bool | None = None) -> None:
    current = get_user_by_id(user_id)
    if not current:
    ...
    if would_remain_admin and would_remain_active:
        return
    if count_admin_users(active_only=True) <= 1:
