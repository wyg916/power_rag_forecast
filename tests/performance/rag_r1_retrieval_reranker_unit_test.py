...
def _provider(
    version: str = "", max_length: int = 128,
    runtime: str = "torch_fp32",
) -> rerank_service.BGETransformersReranker:
    return rerank_service.BGETransformersReranker(
        "fake-reranker-model", "bge-reranker-v2-m3",
        batch_size=8, max_length=max_length, model_version=version, runtime=runtime,
    )
...
def test_score_pairs_uses_configured_effective_max_length(monkeypatch, configured):
    ...
    assert state.max_lengths == [configured]


def test_non_fp32_runtime_is_rejected():
    with pytest.raises(ValueError, match="reranker_runtime_invalid"):
        _provider(runtime="torch_dynamic_int8")
